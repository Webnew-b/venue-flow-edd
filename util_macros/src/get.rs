    ...
    let field_descriptions: proc_macro2::TokenStream = match &ast.data {
        Data::Struct(data_struct) => match &data_struct.fields {
            syn::Fields::Named(field_named) => {
                let field_names =
                    field_named.named.iter().map(|i| i.ident.as_ref().unwrap());

                let field_types = field_named.named.iter().map(|i| &i.ty);
            ...
            syn::Fields::Unnamed(field_unnamed) => {
                let return_types_with_lifetime =
                    field_unnamed.unnamed.iter().map(|f| {
                        let ty = &f.ty;
                        quote! { &'a #ty }
                    });

                let field_ref_access = (0..field_unnamed.unnamed.len())
                    .map(syn::Index::from)
                    .map(|i| quote! { &self.#i });
