...
use crate::repositroy::image_file::{
...
fn verify_image_type(path: &Path) -> io::Result<()> {
    if !path.exists() {
        let e =
            io::Error::new(io::ErrorKind::Interrupted, "File is not found.");
        return Err(e);
    }
...
pub async fn example_upload_oss(
    ...
    if let Err(e) = verify_image_type(&temp_file_path) {
        tracing::error!("{}", e.to_string());
        return Err(CustomResponseError::BadRequest(
            "The file format is not a image.".to_string(),
        ));
    }
    ...
    let temp_file_name = match temp_file_name {
        ...
        None => {
            return Err(CustomResponseError::BadRequest(
                "The file name is invaild.".to_string(),
            ));
        },
    };
    ...
    let ext = match ext {
        ...
        None => {
            return Err(CustomResponseError::BadRequest(
                "The file extension is invaild.".to_string(),
            ));
        },
    };
    ...
    let mut file_path = match PathBuf::from_str(&config.get_temp_folder()) {
        ...
        Err(e) => {
            tracing::error!(
                "Could not get the file path,cause:{}",
                e.to_string()
            );
