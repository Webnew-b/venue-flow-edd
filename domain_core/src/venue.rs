...
mod tests {
    ...
    fn test_update_venue_success() {
        ...
        let clock = MockClock {
            now: Utc.with_ymd_and_hms(2025, 7, 12, 13, 0, 0).unwrap(),
        };
    ...
    fn test_update_venue_fails_on_empty_update() {
        let venue = create_test_venue();
        let clock = MockClock {
            now: Utc.with_ymd_and_hms(2025, 7, 12, 14, 0, 0).unwrap(),
        };
        ...
        assert!(
            result.is_err(),
            "Update should fail when no fields are provided"
        );
        let error = result.err().unwrap();
        assert!(
            matches!(error, DomainCoreError::MustIncludeFieldForUpdate),
            "Error should be MustIncludeFieldForUpdate"
        );
    }
    ...
    fn test_update_venue_fails_on_garde_validation() {
        let venue = create_test_venue();
        let clock = MockClock {
            now: Utc.with_ymd_and_hms(2025, 7, 12, 14, 30, 0).unwrap(),
        };
        ...
        assert!(
            result.is_err(),
            "Update should fail when a field fails validation"
        );
        let error = result.err().unwrap();
        assert!(
            matches!(
                error,
                DomainCoreError::VenueError(VenueError::FieldValidatedFail(_))
            ),
            "Error should be FieldValidatedFail"
        );
    }
    ...
    fn test_update_images() {
        ...
        assert_eq!(
            ...
            2,
            "Venue should have 2 images after update"
        );
        assert_eq!(
