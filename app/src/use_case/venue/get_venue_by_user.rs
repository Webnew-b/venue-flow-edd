...
pub fn tranform_venue(venue:&Venue,lessor:&Lessor) -> AppResult<GetVenueRes> {
    let id = venue.id().ok_or(AppError::IdInexisted("venue".to_string()))?;
    let lessor_id = lessor.id().ok_or(AppError::IdInexisted("lessor".to_string()))?;

    let images:Vec<VenueImageRes> = venue.images()
        .to_vec()
        .clone()
        .into_iter()
        .map(|x| x.into())
    ...
    let res = GetVenueRes {
        ...
        description:venue.description().cloned(),
        lessor_id,
