...
use crate::event_service::email_service::{generate_cancellation_email_simple, generate_cancellation_email_simple_for_organizer, generate_rental_email_simple, EmailService, EmailStruct};
    ...
    pub async fn execute(&self,event:AppEvent) -> EventResult<()> {
        match event {
            ...
            AppEvent::ApprovedRentalRequest { 
                ...
                self.email_service.send_email(organizer_email).await?;
                tracing::info!("Sended ApprovedRentalRequest Email to organizer {}",organizer_id);
                Ok(())
            },
            AppEvent::RejectedRentalRequest { 
                ...
                self.email_service.send_email(organizer_email).await?;
                tracing::info!("Sended RejectedRentalRequest Email to organizer {}",organizer_id);
                Ok(())
            },
